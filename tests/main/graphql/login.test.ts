...
describe('Login GraphQL', () => {
  ...
  describe('SignUp Mutation', () => {
    const query = `mutation {
      signUp (name: "Stephano", email: "stephano@mail.com", password: "123", passwordConfirmation: "123") {
        accessToken
        name
      }
    }`
    ...
    test('Should return an Account on valid data', async () => {
      ...
      expect(res.body.data.signUp.name).toBe('Stephano')
    })
    ...
    test('Should return EmailInUseError on invalid data', async () => {
      ...
      expect(res.body.data).toBeFalsy()
      expect(res.body.errors[0].message).toBe('Email is already in use')
    })
  })
})
